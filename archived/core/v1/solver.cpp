...
extern "C" {
PSolver::PSolver(const std::string& filePath,
                 int minErase,
                 int steps,
                 int size) {
...
std::vector<Route> PSolver::solve() {
    ...
    //     new ColourProfile({pad::fire, pad::water, pad::wood, pad::light,
    //     pad::dark})};
    // Laou
    // std::vector<Profile *> profiles{
    //     new ComboProfile,
    ...
    //     new ComboProfile};
    // new PlusProfile({pad::fire, pad::water, pad::wood, pad::light,
    // pad::dark}), new ColourProfile({pad::light, pad::dark})};
    ...
    // at the same time, the program will crash. By locking and unlocking, it
    // will make sure it is safe
    std::mutex mtx;
    ...
    for (int i = 0; i < steps; ++i) {
                ...
                for (int k = 0; k < threadSize; ++k) {
                    ...
                    if (currentState != nullptr) {
                        ...
                        for (auto& s : currentState->getChildren()) {
                            // Simply insert because states compete with each
                            // other
                            mtx.lock();
        ...
        toVisit = std::priority_queue<PState*, std::vector<PState*>,
                                      PointerCompare>();
