...
struct c_state {
    ...
    c_location routes[pazusoba::MAX_DEPTH + 1];
...
c_state convert(const pazusoba::solver& solver, const pazusoba::state& state) {
    ...
    const int directions[] = {-column, column, -1, 1};
    // get the route from state
    const auto& route = state.route;

    const int max_index = step / pazusoba::ROUTE_PER_LIST;
    // in case, it doesn't fill up the space, check the offset
    const int offset = step % pazusoba::ROUTE_PER_LIST;
    ...
    while (index <= max_index) {
        ...
        int limit = pazusoba::ROUTE_PER_LIST;
        if (index == max_index) {
            limit = offset;
            // shift the number to the left
            curr <<= (pazusoba::ROUTE_PER_LIST - offset) * 3;
        }

        for (int i = 0; i < limit; i++) {
            count++;
            // get first 3 bits and shift to the right, 3 * 20
            curr_loc += directions[(curr & pazusoba::ROUTE_MASK) >> 60];
...
c_state adventure(int argc, char* argv[]) {
    pazusoba::debug_print("Calling from shared library\n");
    for (int i = 0; i < argc; i++) {
        pazusoba::debug_print("argv[%d] = %s\n", i, argv[i]);
    }
